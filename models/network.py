        ...
        while True:
            ...
            if simulate_orphan_blocks:
                ...
                for selected_node in selected_nodes:
                    if selected_node.crashed == False:
                ...
                if selected_node.crashed == False:
                    ...
                    self._build_new_block(selected_node)
                     ...
                     if self.blockchain not in ['vcubechain', 'fabric']:
                         selected_node = scipy.random.choice(
                            self._list_nodes, 1, replace=False, p=self._list_probabilities)[0]
                         self._build_new_block(selected_node)
                     elif self.blockchain =='vcubechain':
                         #selected_node = scipy.random.choice(
                            #self._list_nodes, 1, replace=False, p=self._list_probabilities)[0]
                         node = selected_node.new_leader()
                         print(f'Crash at {time(self.env)}: Node {node} selected')
                         self._build_new_block(self.get_node(node))
                     else:
                         node = selected_node.new_leader()
                         print(f'Crash at {time(self.env)}: Node {node} selected as leader')
                         self._build_new_block(self.get_node(node))
