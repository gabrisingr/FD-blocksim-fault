...
class FabricNode(Node):
    ...
    def new_leader(self):
        if (self.crashed == True):
            new_leader = str(max(self.gossip.correct, key=int))
    ...
    def _send_leader(self, _from: str, leader: str):
        ...
        if self.address == _from:
            ...
            ss = self.gossip.neighbors(int(self.address), int(_from))
        for p in self.gossip.neighbors(ss):
            print(
   ...
   ##              ##
    ...
    def notify_crash(self, p: str):
        ...
        new_neighbor = self.gossip.neighbor(self.address, self.gossip.cluster(int(self.address), int(p)))
        print(
